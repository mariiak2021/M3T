...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/mot_challenge/'),  # Trackers location 'data/trackers/mot_challenge/'
            'OUTPUT_FOLDER': '/home/mkhan/sort/TrackEval/TRACKERS_FOLDER/',  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'CLASSES_TO_EVAL': ['ArmChair', 'Book', 'Boots', 'Bowl', 'Box', 'Cabinet', 'Candle', 'CellPhone', 'Chair', 'CoffeeTable', 'CreditCard', 'Desk', 'DeskLamp', 'DiningTable', 'DogBed', 'Drawer', 'Dresser', 'FloorLamp', 'GarbageCan', 'HousePlant', 'KeyChain', 'Laptop', 'Newspaper', 'Ottoman', 'Painting', 'Pen', 'Pencil', 'Pillow', 'Plate', 'RemoteControl', 'Safe', 'Shelf', 'ShelvingUnit', 'SideTable', 'Sofa', 'Statue', 'Stool', 'Television', 'TissueBox', 'TVStand', 'Vase', 'Watch'],  # Valid: ['pedestrian']
            'BENCHMARK': 'ai2thor',  # Valid: 'MOT17', 'MOT16', 'MOT20', 'MOT15'
            'SPLIT_TO_EVAL': 'all',  # Valid: 'train', 'test', 'all'
            'INPUT_AS_ZIP': False,  # Whether tracker input files are zipped
            'PRINT_CONFIG': True,  # Whether to print current config
            'DO_PREPROC': True,  # Whether to perform preprocessing (never done for MOT15)
            'TRACKER_SUB_FOLDER': 'data2',  # Tracker files are in TRACKER_FOLDER/tracker_name/TRACKER_SUB_FOLDER
    ...
    def __init__(self, config=None):
        ...
        self.valid_classes = []
        labelsPath = ("/home/mkhan/sort/obj1_416.names")
        ...
        ##data = np.loadtxt(curr_file, delimiter=',')
        #print (data[:,2])
        ...
        for label in LABELS:
            ...
            self.class_name_to_class_id[label] = LABELS.index(label)
            ##if LABELS.index(label) in classes:
            self.valid_classes.append(label)
        #print (self.valid_classes)

        self.config['CLASSES_TO_EVAL'] = self.valid_classes
        #print (self.class_name_to_class_id)
        #print (self.valid_classes)
        #print (self.config['CLASSES_TO_EVAL'])
        ...
        if self.config['TRACKERS_TO_EVAL'] is None:#/home/mkhan/sort/TrackEval/data/trackers/mot_challenge/ai2thor-all/detection/data
            self.tracker_list = os.listdir(self.tracker_fol)#os.listdir(os.path.join(self.tracker_fol, 'detection', 'data'))
        else:
                    ...
                    if is_gt:
                        ...
                        raise TrackEvalException(
                            'Cannot convert tracking data from tracker %s, sequence %s to float. Is data corrupted?' % (
                                tracker, seq))
                try:
                    raw_data['dets'][t] = np.atleast_2d(time_data[:, 3:7])
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        #print ("claases info", cls, self.class_name_to_class_id)
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            #print ("tracker_classes", tracker_classes)
            ...
            if self.do_preproc and self.benchmark != 'MOT15':
                ...
                tr_to_keep_mask = (np.equal(tracker_classes, cls_id))
            else:
                #print ("not2")
                # There are no classes for MOT15
                gt_to_keep_mask = np.not_equal(gt_zero_marked, 0)
            data['gt_ids'][t] = gt_ids[gt_to_keep_mask]
            #print ("data['gt_ids'][t]", data['gt_ids'][t])
            data['gt_dets'][t] = gt_dets[gt_to_keep_mask, :]
            #print ("simil", similarity_scores, tr_to_keep_mask, t, similarity_scores[t,tr_to_keep_mask] )
            #data['similarity_scores'][t] = similarity_scores[t,tr_to_keep_mask] 
            
            data['tracker_ids'][t] = tracker_ids[tr_to_keep_mask]
            #print ("data['tracker_ids'][t]", data['tracker_ids'][t])
            data['tracker_dets'][t] = tracker_dets[tr_to_keep_mask, :]
            data['tracker_confidences'][t] = tracker_confidences[tr_to_keep_mask]
            #print ("similarity_scores", similarity_scores[:, tr_to_keep_mask])
            similarity_scores = similarity_scores[:, tr_to_keep_mask]
            
            #print (similarity_scores.shape)
            data['similarity_scores'][t] = similarity_scores[gt_to_keep_mask] #,tr_to_keep_mask
            #print ("similarity_scores updates", data['similarity_scores'][t], data['similarity_scores'][t].shape)
